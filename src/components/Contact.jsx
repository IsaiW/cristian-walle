      ...
      <div className="container lg:grid lg:grid-cols-2 lg:items-stretch">
        <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
          ...
          <p className="text-zinc-400 mt-3 mb-8 max-w-[50ch] lg:max-w-[30ch] reveal-up">
            Open to data engineering roles, full-stack projects, and freelance work. Drop a message — I reply fast.
          </p>
          <div className="flex items-center gap-2 mt-auto">
            {socialLinks.map(({ href, icon }, key) => (
              <a key={key} href={href} target="_blank" className="w-12 h-12 grid place-items-center ring-inset ring-1 ring-zinc-800 rounded-lg transition-[background-color] duration-200 hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80 reveal-up" rel="noopener noreferrer">
                {icon}
              </a>
            ))}
          </div>
        </div>
        <form action="https://getform.io/f/akkyjgda" method="POST" className="xl:pl-10 2xl:pl-20">
          <div className="md:grid md:items-center md:grid-cols-2 md:gap-2">
            <div className="mb-4">
              <label htmlFor="name" className="label reveal-up">Name</label>
              <input type="text" name="name" autoComplete="name" required placeholder="Cristian Walle" id="name" className="text-field reveal-up" />
            </div>
            <div className="mb-4">
              <label htmlFor="email" className="label reveal-up">Email</label>
              <input type="email" name="email" autoComplete="email" required placeholder="cristianisai.wg@gmail.com" id="email" className="text-field reveal-up" />
            </div>
          </div>
          <div className="mb-4">
